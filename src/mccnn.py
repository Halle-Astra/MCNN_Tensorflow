...
csz = (1/4)#change size 的比例

def shallow_net_9x9(x):
	net = L.conv(x, name="conv_sn9x9_1", kh=9, kw=9, n_out=16)	#相当于自己搭建了一个keras，你太优秀了吧。。。
...
def shallow_net_7x7(x):
	...
	net = L.pool(net, name="pool_sn7x7_2", kh=2, kw=2, dw=2, dh=2)			   #这部分代码可修改优化更简单些
...
def build(input_tensor, norm = False):
	"""
	Builds the entire multi column cnn with 3 shallow nets with different input kernels and one fusing layer.  #fusing融合
